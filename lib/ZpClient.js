...
class ZpClient extends events.EventEmitter {
  ...
  static parseMember (member) {
    const props = {
      ...
      physicalControls: member.zoneName !== 'BOOST',
  ...
  get physicalControls () { return this._props.physicalControls }
  ...
  get info () {
    return {
      ...
      name: this.name,
      physicalControls: this.physicalControls,
      role: this.role,
  ...
  async init () {
    ...
    zoneGroupState.zoneGroups.forEach((group) => {
      group.zoneGroupMembers.forEach((member) => {
        ...
        if (member.invisible !== 1 || props.name === 'BOOST') {
