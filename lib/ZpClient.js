...
class ZpClient extends events.EventEmitter {
  ...
  static meta (fav) {
    ...
    meta += `<item id="${fav.resMD.id}" parentID="${fav.resMD.parentId}" restricted="${fav.resMD.restricted}">`
    meta += `<dc:title>${fav.resMD.title}</dc:title>`
    meta += `<upnp:class>${fav.resMD.class}.sonos-favorite</upnp:class>`
    if (fav.albumArtUri != null) {
      meta += `<upnp:albumArtURI>${he.escape(fav.albumArtUri)}</upnp:albumArtURI>`
    }
    meta += `<r:description>${fav.description}</r:description>`
    meta += `<desc id="cdudn" nameSpace="urn:schemas-rinconnetworks-com:metadata-1-0/">${fav.resMD.desc._}</desc>`
    meta += '</item></DIDL-Lite>'
