...
class ZpClient extends events.EventEmitter {
  ...
  async post (device, service, action, options) {
    ...
    this._debugRequest(
      ...
      this._jsonFormatter.stringify(responseBody)
    )
    return responseBody
  }
  ...
  async get (url = '/xml/device_description.xml') {
    ...
    this._debugRequest(
      'get(%j) --> %s', url, this._jsonFormatter.stringify(response)
    )
    return response
  }
    ...
    if (this._client == null) {
      ...
      this._client
        .on('error', (error) => {
          // FIXME
          this.emit(
            'error', error, error.request.id, error.request.method,
            error.request.resource, error.request.body, error.request.url)
        })
        // this._client.on('request', (request) => {
        //   this.emit('request', id, method, resource, body, url)
        // })
        // this._client.on('response', (response) => {
