...
const ZpXmlParser = require('./XmlParser')
    ...
    this._server = http.createServer((request, response) => {
      ...
      request.on('end', async () => {
        ...
        if (request.method === 'GET' && request.url === '/notify') {
          ...
          const device = array[5] == null ? 'ZonePlayer' : array[3]
          const service = array[5] == null ? array[3] : array[4]
          if (array[1] === 'notify' && client !== null && service !== null) {
            const json = await this._parser.parse(request.body)
            this._debug('%s %s event: %j', device, service, json)
            client.emit('event', device, service, json)
          }
        }
        response.end()
      })
...
class ZpClient extends events.EventEmitter {
  ...
  constructor (options) {
    ...
    this._zpListener.addClient(this._id, this)
  }
  ...
  async subscribe (url, sid) {
    const callbackUrl = await this._zpListener.callbackUrl() +
                        '/' + this._id + url
