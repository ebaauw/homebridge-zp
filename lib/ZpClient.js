...
class ZpClient extends homebridgeLib.HttpClient {
  ...
  async init () {
    ...
    this._props.address = (await dns.lookup(this._params.hostname)).address
  ...
  async initTopology (zpClient = {}) {
    ...
    this._zonesByName = {}
    if (
      zpClient.zoneGroupState != null &&
      zpClient.zoneGroupState.zoneGroups != null
    ) {
      this._zoneGroupState = zpClient.zoneGroupState
      this._zoneGroupState.museHouseholdId = zpClient.household
    } else {
      try {
        this._zoneGroupState = await this.getZoneGroupState()
        const attrs = await this.getZoneGroupAttributes()
        this._zoneGroupState.museHouseholdId = attrs.currentMuseHouseholdId
      } catch (error) {
        if (error.statusCode === 500) {
          ...
          await this.open(this._params.listener)
    ...
    if (this._zoneGroupState != null) {
      const household = this._zoneGroupState.museHouseholdId != null
        ? this._zoneGroupState.museHouseholdId.split('.')[0]
        : undefined
      for (const group of this._zoneGroupState.zoneGroups) {
        ...
        groupMemberNames = [groupName].concat(groupMemberNames.sort())
        groupName = groupMemberNames.join(' + ')
        for (const id of ids) {
  ...
  /** The number of seconds since the last communication from the zone player,
    * i.e. the time elapsed since the most recent push notification or request
