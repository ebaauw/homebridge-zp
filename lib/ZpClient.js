...
class ZpClient extends events.EventEmitter {
  // Normalise IPv4 address (remove leading zeroes).
  ...
  constructor (options) {
    ...
    this._subscriptionTimeout = 30 * 60 // TODO
  ...
  async open (zpListener) {
    ...
    for (const device of deviceList) {
      for (const service of device.serviceList) {
        const serviceName = service.scpdUrl.substr(5, service.scpdUrl.length - 10)
  ...
  async stop () {
    return this._post('MediaRenderer', 'AVTransport', 'Stop', {
      InstanceID: 0
    })
  }

  async play () {
    return this._post('MediaRenderer', 'AVTransport', 'Play', {
      InstanceID: 0,
      Speed: 1
    })
  }
  ...
  async getZoneAttributes () {
    return this._post('ZonePlayer', 'DeviceProperties', 'GetZoneAttributes', {})
  }

  async getZoneInfo () {
    return this._post('ZonePlayer', 'DeviceProperties', 'GetZoneInfo', {})
  }

  async getZoneGroupAttributes () {
    return this._post('ZonePlayer', 'ZoneGroupTopology', 'GetZoneGroupAttributes', {})
  }

  async listAlarms () {
    return this._post('ZonePlayer', 'AlarmClock', 'ListAlarms', {})
  }

  async getVolume (channel = 'Master') {
    return (await this._post('MediaRenderer', 'RenderingControl', 'GetVolume', {
      InstanceID: 0,
      Channel: channel
    })).currentVolume
  }
  ...
  // POST a SOAP action.
  async _post (device, service, action, options) {
    this._debug('_post(%j, %j, %j, %j)', device, service, action, options)
    let body = '<s:Envelope '
