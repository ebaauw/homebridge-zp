...
class ZpClient extends events.EventEmitter {
  ...
  static parseMember (member) {
    ...
    if (map != null) {
      if (map.ids[0] === props.id) {
        ...
        assert.strictEqual(map.ids[1], props.id)
        props.role = slave
        props.channel = map.channels[1]
  ...
  async _request (requestObj) {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.emit(
        'request', requestId, method, requestObj.url,
      ...
      request(requestObj, (error, response) => {
        ...
        this._props.lastSeen = new Date()
        this.emit('response', requestId, response.statusCode)
