  ...
  attachZpClient () {
    ...
    this.zpClient
      ...
      .on('response', (response) => {
        ...
        if (response.rawBody != null) {
          this.vvdebug(
            ...
            response.headers, response.rawBody
          )
          this.vdebug(
            'request %s: response: %j', response.request.id, response.body
          )
        }
      })
      .on('error', (error) => {
        ...
        if (error.request.id !== this.requestId) {
          this.log(
            'request %s: %s %s%s', error.request.id,
      ...
      .on('message', (message) => {
        ...
        this.vvdebug('notify %s/Event: %s', notify, message.rawBody)
        this.vdebug('notify %s/Event: %j', notify, message.body)
