...
function ZpPlatform (log, config) {
  ...
  this.resetTimeout = config.resetTimeout || 500 // milliseconds
...
ZpPlatform.prototype.findPlayers = function () {
  SonosModule.search({ timeout: this.searchTimeout }, (zp, model) => {
    ...
    zp.model = model
    zp.deviceDescription((error, data) => {
      if (error) {
        this.log.error('%s: error %s', zp.host, error)
      } else {
        // this.log.debug('%s: device description: %j', zp.host, data)
        zp.id = data.UDN.substr(5)
        zp.modelName = data.modelName + ' (' + model + ')'
        deviceProperties.GetZoneAttributes({}, (err, attrs) => {
          if (err) {
            this.log.error('%s: error %s', zp.host, err)
          } else {
...
ZpPlatform.prototype.setPlatformCoordinator = function (group) {
  this.coordinator = group
  for (const id in this.zpAccessories) {
    const accessory = this.zpAccessories[id]
    accessory.groupService.updateCharacteristic(
      my.Characteristic.SonosCoordinator, accessory === group
    )
  }
}
