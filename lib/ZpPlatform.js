...
import { MdnsClient } from 'homebridge-lib/MdnsClient'
import { OptionParser } from 'homebridge-lib/OptionParser'
import { Platform } from 'homebridge-lib/Platform'
import { UpnpClient } from 'homebridge-lib/UpnpClient'
  ...
  constructor (log, configJson, homebridge) {
    ...
    this
      ...
      .on('shutdown', async () => {
        ...
        this.mdnsClient?.stopListen()
        this.upnp?.stopListen()
      })
    ...
    if (this.config.mdns) {
      ...
      this.mdnsClient
        .on('serviceUp', (address, message) => {
          this.handleMdnsMessage(message)
        })
        .listen()
    }
    // Setup listener for UPnP announcements.
    this.upnp = new UpnpClient({
      deviceType: 'urn:schemas-upnp-org:device:ZonePlayer:1',
      logger: this
    })
    this.upnp
      .on('deviceAlive', this.handleUpnpMessage.bind(this))
      .on('deviceFound', this.handleUpnpMessage.bind(this))
      .listen()
    ...
    zpClient
      ...
      .on('error', (error) => {
        ...
        this.log(
          ...
          error.request.action == null ? '' : ' ' + error.request.action
        )
        this.warn(
