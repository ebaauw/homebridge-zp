...
class ZpPlatform extends homebridgeLib.Platform {
  ...
  async createZpClient (address, id) {
    ...
    if (this.zpClients[id] == null) {
      ...
      zpClient.on('response', (id, status) => {
        this.debug(
          '%s: request %s: status %d',
          zpClient.name == null ? zpClient.address : zpClient.name,
          id, status
        )
      })
  ...
  logTopology () {
    ...
    for (const zonePlayerName of Object.keys(this.zonePlayersByName).sort()) {
      ...
      const expose = !this.config.excludeAirplay || !zonePlayer.airPlay
      if (zonePlayer.role === 'master') {
        if (expose && this.zpMasters[zonePlayer.id] == null) {
          this.zpMasters[zonePlayer.id] = new ZpAccessory.Master(this, params)
        }
        if (expose && this.config.tv && this.zpTvs[zonePlayer.id] == null) {
