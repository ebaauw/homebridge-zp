...
function ZPAccessory(platform, zp) {
  ...
  this.service.addOptionalCharacteristic(Characteristic.AudioVolume);
  this.service.getCharacteristic(Characteristic.AudioVolume)
    .on("get", function(callback) {callback(null, this.state.volume);}.bind(this))
    .on("set", this.setVolume.bind(this));
  this.service.addOptionalCharacteristic(Characteristic.AudioMute);
  this.service.getCharacteristic(Characteristic.AudioMute)
  ...
  this.parser.parseString(data.LastChange, function(err, json) {
    ...
    if (event.Volume) {
      ...
      if (volume !== this.state.volume) {
        ...
        this.service.setCharacteristic(Characteristic.AudioVolume, this.state.volume);
      }
    }
    if (event.Mute) {
      ...
      if (mute !== this.state.mute) {
        ...
        this.service.setCharacteristic(Characteristic.AudioMute, this.state.mute);
      }
    }
  }.bind(this));
};
