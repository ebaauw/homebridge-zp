...
function ZPAccessory (platform, zp) {
  ...
  this.groupService.addOptionalCharacteristic(Characteristic.ChangeVolume)
  this.groupService.getCharacteristic(Characteristic.ChangeVolume)
    .updateValue(0)
  ...
  this.zoneService.addOptionalCharacteristic(Characteristic.ChangeVolume)
  this.zoneService.getCharacteristic(Characteristic.ChangeVolume)
    .updateValue(0)
...
ZPAccessory.prototype.handleAVTransportEvent = function (data) {
  ...
  this.parser.parseString(data.LastChange, (err, json) => {
    ...
    if (track !== this.state.group.track &&
      ...
      this.log.info(
        '%s: set current track from "%s" to "%s"', this.name,
...
ZPAccessory.prototype.setZoneChangeVolume = function (volume, callback) {
  ...
  setTimeout(() => {
    ...
    this.zoneService.updateCharacteristic(Characteristic.ChangeVolume, 0)
...
ZPAccessory.prototype.setGroupChangeVolume = function (volume, callback) {
  ...
  setTimeout(() => {
    ...
    this.groupService.updateCharacteristic(Characteristic.ChangeVolume, 0)
  }, this.platform.resetTimeout)
