...
function ZpAccessory (platform, zp) {
  ...
  this.on('ZoneGroupTopology', this.handleZoneGroupTopologyEvent)
  ...
  this.subscribe('ZoneGroupTopology', (err) => {
    if (err) {
      this.log.error('%s: subscribe to ZoneGroupTopology events: %s', this.name, err)
    }
  })
}
...
ZpAccessory.prototype.getServices = function () {
  ...
  this.hasBalance = stereoModels.includes(this.zp.model) ||
    this.capabilities.stereoPair

  this.infoService = new Service.AccessoryInformation()
  this.infoService
    .updateCharacteristic(Characteristic.Manufacturer, 'homebridge-zp')
    .updateCharacteristic(Characteristic.Model, this.zp.modelName)
...
ZpAccessory.prototype.handleDevicePropertiesEvent = function (data) {
  this.log.debug('%s: DeviceProperties event', this.name)
  if (this.capabilities == null) {
    ...
    this.log.warn(
      ...
      this.name, this.zp.modelName, caps.airPlay, caps.audioIn, caps.tvIn
    )
    this.log.warn(
...
ZpAccessory.prototype.handleZoneGroupTopologyEvent = function (data) {
  this.log.debug('%s: ZoneGroupTopology event', this.name)
  if (data.ZoneGroupState != null) {
    this.parser.parseString(data.ZoneGroupState, (error, json) => {
      if (error) {
        return
      }
      const zoneGroupState = json.ZoneGroupState
      // this.log.debug('%s: ZoneGroupState: %j', this.name, zoneGroupState)
      if (
        Array.isArray(zoneGroupState.ZoneGroups) &&
        zoneGroupState.ZoneGroups[0] != null &&
        Array.isArray(zoneGroupState.ZoneGroups[0].ZoneGroup)
      ) {
        for (const zoneGroup of zoneGroupState.ZoneGroups[0].ZoneGroup) {
          if (Array.isArray(zoneGroup.ZoneGroupMember)) {
            for (const zoneGroupMember of zoneGroup.ZoneGroupMember) {
              const member = zoneGroupMember.$
              if (member != null) {
                const slave = member.Invisible == null ? '' : ' (slave)'
                const ipAddress = member.Location.split('/')[2].split(':')[0]
                this.log.debug(
                  '%s: found %s at %s in %s%s', this.name,
                  member.UUID, ipAddress, member.ZoneName, slave
                )
                if (this.platform.zpAccessories[member.UUID] != null) {
                  this.platform.zpAccessories[member.UUID].found()
                }
              }
              if (Array.isArray(zoneGroupMember.Satellite)) {
                for (const satellite of zoneGroupMember.Satellite) {
                  const member = satellite.$
                  if (member != null) {
                    const ipAddress = member.Location.split('/')[2].split(':')[0]
                    this.log.debug(
                      '%s: found %s at %s in %s (satellite)', this.name,
                      member.UUID, ipAddress, member.ZoneName
                    )
                  }
                }
              }
      ...
      if (Array.isArray(zoneGroupState.VanishedDevices)) {
        for (const vanishedDevice of zoneGroupState.VanishedDevices) {
          if (Array.isArray(vanishedDevice.Device)) {
            const device = vanishedDevice.Device[0].$
            if (device != null) {
              this.log.debug('%s: lost %s in %s', this.name, device.UUID, device.ZoneName)
              if (this.platform.zpAccessories[device.UUID]) {
                this.platform.zpAccessories[device.UUID].lost()
              }
            }
          }
        }
  ...
  this.parser.parseString(data.LastChange, (err, json) => {
    ...
    if (this.tv && on != null) {
      ...
      if (tv !== this.state.group.tv) {
        if (tv || this.state.group.tv == null) {
          this.log.info(
...
ZpAccessory.prototype.subscribe = function (service, callback) {
  if (this.platform.shuttingDown || this.subscriptions[service] != null) {
    return callback()
  }
...
ZpAccessory.prototype.found = function () {
  if (this.isLost == null) {
    return
  }
  this.log('%s: found', this.name)
  delete this.isLost
  this.createSubscriptions()
}

ZpAccessory.prototype.lost = function () {
  if (this.isLost) {
    return
  }
  this.isLost = true
  this.log('%s: lost', this.name)
  for (const service in this.subscriptions) {
    this.log.debug(
      '%s: invalidate %s subscription %s', this.name, service,
