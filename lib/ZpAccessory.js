...
ZpAccessory.prototype.handleRenderingControlEvent = function (data) {
  ...
  this.parser.parseString(data.LastChange, (err, json) => {
    ...
    if (event.NightMode) {
      if (this.state.zone.nightMode == null) {
        this.zoneService.addOptionalCharacteristic(my.Characteristic.NightMode)
        this.zoneService.getCharacteristic(my.Characteristic.NightMode)
          .on('set', this.setZoneNightMode.bind(this))
      }
      const nightMode = event.NightMode[0].$.val === '1'
      if (nightMode !== this.state.zone.nightMode) {
        this.log.info('%s: set night mode from %s to %s', this.name, this.state.zone.nightMode, nightMode)
        this.state.zone.nightMode = nightMode
        this.zoneService.updateCharacteristic(my.Characteristic.NightMode, this.state.zone.nightMode)
      }
    }
    if (event.DialogLevel) {
      if (this.state.zone.dialogLevel == null) {
      ...
      const dialogLevel = event.DialogLevel[0].$.val === '1'
      if (dialogLevel !== this.state.zone.dialogLevel) {
        this.log.info('%s: set speech enhancement from %s to %s', this.name, this.state.zone.dialogLevel, dialogLevel)
        this.state.zone.dialogLevel = dialogLevel
        this.zoneService.updateCharacteristic(my.Characteristic.SpeechEnhancement, this.state.zone.dialogLevel)
      }
    }
  })
}
...
// Called by homebridge when characteristic is changed from homekit.
ZpAccessory.prototype.setZoneNightMode = function (nightMode, callback) {
  nightMode = !!nightMode
  if (this.state.zone.nightMode === nightMode) {
    return callback()
  }
  this.log.info('%s: night mode changed from %s to %s', this.name, this.state.zone.nightMode, nightMode)
  const args = {
    InstanceID: 0,
    EQType: 'NightMode',
    DesiredValue: nightMode ? '1' : '0'
  }
...
// Called by homebridge when characteristic is changed from homekit.
ZpAccessory.prototype.setZoneSpeechEnhancement = function (dialogLevel, callback) {
  dialogLevel = !!dialogLevel
  if (this.state.zone.dialogLevel === dialogLevel) {
    return callback()
  }
  this.log.info('%s: speech engancement changed from %s to %s', this.name, this.state.zone.dialogLevel, dialogLevel)
  const args = {
    InstanceID: 0,
    EQType: 'DialogLevel',
    DesiredValue: dialogLevel ? '1' : '0'
  }
  this.renderingControl._request('SetEQ', args, (err, status) => {
    if (err) {
