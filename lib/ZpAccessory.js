...
class ZpAccessory extends homebridgeLib.AccessoryDelegate {
  constructor (platform, params) {
    ...
    if (this.platform.config.speakers) {
      ...
      // TODO remove sercice if restored
    }
    if (this.platform.config.leds) {
      this.ledService = new ZpService.Led(this)
    } else {
      // TODO remove sercice if restored
    }
  ...
  handleZonePlayerGroupManagementEvent (event) {
    // this.debug('GroupRenderingControl event %j', event)
  ...
  async handleZonePlayerAlarmClockEvent (event) {
    // this.debug('AlarmClock event %j', event)
    // if (event.alarmListVersion === this.platform.alarmListVersion) {
    //   // Already handled.
    //   return
    // }
    // this.platform.alarmListVersion = event.alarmListVersion
    const alarms = (await this.zpClient.listAlarms()).currentAlarmList
      .filter((alarm) => { return alarm.roomUuid === this.context.id })
    this.debug('alarms: %j', alarms)
