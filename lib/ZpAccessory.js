...
function ZpAccessory (platform, zp) {
  ...
  if (this.platform.tv) {
    this.televisionService = new Service.Television(this.name + ' TV')
    this.televisionService.getCharacteristic(Characteristic.ConfiguredName)
      .updateValue(this.name)
      .on('set', (value, callback) => {
        this.log('%s: configured name changed to %j', this.name, value)
        callback()
      })
    ...
    this.televisionService.getCharacteristic(Characteristic.Active)
      .on('set', (value, callback) => {
        this.log('%s: active changed to %j', this.name, value)
        callback()
      })
    this.televisionService.getCharacteristic(Characteristic.ActiveIdentifier)
      ...
      .setValue(1)
      .on('set', (value, callback) => {
        this.log('%s: active identifier changed to %j', this.name, value)
        callback()
      })
    this.televisionService.getCharacteristic(Characteristic.RemoteKey)
      .on('set', (value, callback) => {
        switch (value) {
          ...
          default:
            this.log('%s: %s (%j)', this.name, remoteKeys[value], value)
            return callback()
        }
      })
    this.services.push(this.televisionService)

    this.televisionSpeakerService = new Service.TelevisionSpeaker(this.name, '1')
    this.televisionSpeakerService
      .updateCharacteristic(Characteristic.Active, Characteristic.Active.ACTIVE)
    ...
    this.televisionSpeakerService.getCharacteristic(Characteristic.VolumeSelector)
      .on('set', (value, callback) => {
        // this.log('%s: Volume %s (%j)', this.name, volumeSelectors[value], value)
    ...
    this.televisionSpeakerService.getCharacteristic(Characteristic.Volume)
      .on('set', (value, callback) => {
        this.log('%s: volume changed to %j', this.name, value)
        callback()
      })
    ...
    this.inputService1
      ...
      .updateCharacteristic(Characteristic.IsConfigured, Characteristic.InputDeviceType.AUDIO_SYSTEM)
      .updateCharacteristic(Characteristic.CurrentVisibilityState, Characteristic.InputDeviceType.AUDIO_SYSTEM)
    this.services.push(this.inputService1)
    this.televisionService.addLinkedService(this.inputService1)
    ...
    this.televisionService.addLinkedService(this.inputService2)
  ...
  this.parser.parseString(data.LastChange, (err, json) => {
    ...
    if (this.tv && this.on != null) {
      ...
      if (tv !== this.state.group.tv) {
        ...
        }, tv || this.state.group.tv == null ? 0 : 10000)
