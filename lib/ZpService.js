...
class Sonos extends ZpService {
  ...
  handleMediaRendererGroupRenderingControlEvent (message) {
    if (message.groupVolumeChangeable === 1) {
      this.zpAccessory.coordinator = this.zpAccessory
...
class Speaker extends ZpService {
  constructor (zpAccessory, params = {}) {
    ...
    if (this.zpClient.zoneDisplayName.endsWith('+Sub)')) {
      this.addCharacteristicDelegate({
        key: 'subEnabled',
...
class Tv extends ZpService {
  ...
  async handleMediaServerContentDirectoryEvent (message) {
    if (
      message.favoritesUpdateId == null ||
      message.favoritesUpdateId === this.favoritesUpdateId
    ) {
      return
    }
    this.favoritesUpdateId = message.favoritesUpdateId
    this.sources = []
