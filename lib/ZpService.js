...
class Sonos extends ZpService {
  ...
  handleMediaRendererAVTransportEvent (payload) {
    ...
    // this.debug('AVTransport event %j', event)
    let on
    let track
  ...
  handleZonePlayerHTControlEvent (event) {
    this.debug('HTControl event: %j', event)
  }
}
...
class Speaker extends ZpService {
  ...
  handleMediaRendererRenderingControlEvent (payload) {
    ...
    const event = payload.lastChange[0]
    // this.debug('RenderingControl event: %j', event)
...
class Alarm extends ZpService {
  constructor (zpAccessory, alarm) {
    const params = {
      id: alarm.id,
      name: zpAccessory.zpClient.zoneName + ' Alarm ' + alarm.id,
  ...
  set alarm (alarm) {
    this._alarm = alarm
    this.values.enabled = alarm.enabled === 1
...
class Led extends ZpService {
  constructor (zpAccessory, params = {}) {
    params.name = zpAccessory.zpClient.zoneName + ' LED'
    params.Service = zpAccessory.Service.hap.Lightbulb
    params.subtype = 'led'
    super(zpAccessory, params)
    this.addCharacteristic({
      key: 'on',
      Characteristic: this.Characteristic.hap.On,
      // getter: this.zpClient.getLedState.bind(this.zpClient),
      setter: this.zpClient.setLedState.bind(this.zpClient)
    })
    this.addCharacteristic({
      key: 'locked',
      Characteristic: this.Characteristic.hap.LockPhysicalControls,
      // getter: async (value) => {
