...
class Tv extends ZpService {
      ...
      setter: async (value) => {
        try {
          ...
          if (value === 1 && source.uri == null) {
            ...
            delete this.zpMaster.coordinator.sonosService.awaitAvTransportEvent
  ...
  nextIdentifier (value) {
    ...
    } while (!this.sources[identifier - 1].visible)
    return identifier
  }
  ...
  updateGroupInputSource (silent = false) {
    ...
    let uri
    let visible = false
    if (this.sonosValues.sonosGroup !== this.zpClient.zoneName) {
  ...
  configureInputSource (configuredName, uri, visible = true, container, meta) {
    this.sources.push({
...
class TvSpeaker extends ZpService {
  constructor (zpAccessory, params = {}) {
    ...
    const volumeSelector = this.addCharacteristicDelegate({
      ...
      this.sonosService.characteristicDelegate('changeVolume').setValue(
    ...
    this.addCharacteristicDelegate({
      ...
      this.sonosService.characteristicDelegate('mute').setValue(value)
    })
    this.emit('initialised')
  }
}
