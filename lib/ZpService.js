...
class Sonos extends ZpService {
  constructor (zpAccessory, params = {}) {
    ...
    this.addCharacteristicDelegate({
      ...
      setter: async (value) => {
        this.warn('repeat: %s, valid modes: %j', value, this.values.currentValidPlayModes)
        try {
          if (
            value === 0 || this.values.currentValidPlayModes.includes(
    ...
    this.addCharacteristicDelegate({
      ...
      setter: async (value) => {
        try {
          if (!value || this.values.currentValidPlayModes.includes('SHUFFLE')) {
    ...
    this.addCharacteristicDelegate({
      ...
      setter: async (value) => {
        try {
          if (!value || this.values.currentValidPlayModes.includes('CROSSFADE')) {
    ...
    this.addCharacteristicDelegate({
      ...
      value: zpAccessory.platform.logLevel
    }).on('didSet', (value) => {
      zpAccessory.logLevel = value
    })
    this.addCharacteristicDelegate({
      key: 'configuredName'
    })
  ...
  handleMediaRendererAVTransportEvent (payload) {
    ...
    if (this.awaitAvTransportEvent) {
      this.debug('ignore AVTransPortEvent after SetAVTransportURI')
      delete this.awaitAvTransportEvent
      this.emit('AVTransportEvent')
      return
    }
...
class Tv extends ZpService {
  constructor (zpAccessory, params = {}) {
    ...
    this.once('initialised', () => {
      this.sonosService.characteristicDelegate('sonosGroup')
        .on('didSet', (value) => {
      ...
      setter: async (value) => {
        try {
          ...
          if (value === 1 && source.uri == null) {
            ...
            if (this.awaitAvTransportEvent) {
              await events.once(zp.sonosService, 'AVTransportEvent')
            }
    ...
    this.addCharacteristicDelegate({
      ...
      this.sonosValues.configuredName = value
    })
    this.values.configuredName = this.sonosValues.configuredName
    this.addCharacteristicDelegate({
  ...
  async handleMediaServerContentDirectoryEvent (event) {
    ...
    const favs = await this.zpClient.browse('FV:2')
    for (const key in favs) {
      const fav = favs[key]
      this.configureInputSource(key, fav.uri, true, fav.container, fav.meta)
    }
    for (let index = this.sources.length; index < nInputSources - 1; index++) {
  ...
  updateGroupInputSource (silent = false) {
    ...
    if (
      ...
      configuredName = 'Join ' + zpClient.zoneName
...
class TvSpeaker extends ZpService {
  constructor (zpAccessory, params = {}) {
    ...
    const service = params.master.speakerService == null
      ? params.master.sonosService : params.master.speakerService
    this.addCharacteristicDelegate({
...
class TvInputSource extends ZpService {
  constructor (zpAccessory, params = {}) {
    ...
    this.addCharacteristicDelegate({
      ...
      silent: true,
      value: this.Characteristics.hap.IsConfigured.CONFIGURED
    })
    this.addCharacteristicDelegate({
    ...
    this.addCharacteristicDelegate({
      ...
      if (this.tvService.sources[this.values.identifier - 1].visible) {
        ...
        setTimeout(() => {
          this.values.targetVisibilityState =
            this.Characteristics.hap.TargetVisibilityState.HIDDEN
        }, this.platform.config.resetTimeout)
      }
    })
