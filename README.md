...
The homebridge-zp plug-in creates web server to receive events from the Sonos ZonePlayers.  The IP address and port number for this listener are logged in a debug message, e.g. `[ZP] listening on http://_<IP address>_:_<port>_/notify`.  To check whether the listener is reachable from the network, open this URL in your web browser.

For each zone, the homebridge-zp plug-in logs a debug message with the zone name and the type, ID and IP address and port of the corresponding ZonePlayer, e.g. `Living Room: setup ZPS9 player RINCON_5CAAFD_xxxxxx_01400 at _<IP address>_:1400`.  To check whether the ZonePlayer has accepted the subscriptions to send notification events to homebridge-zp, open `http://_<IP address>_:1400/status` in your web browser to see the Options for the ZonePlayer.  Select `upnp` and then select `Incoming subscriptions`.  Next to the subscriptions from other ZonePlayers and from Sonos apps, you should find the subscriptions from homebridge-zp.  Note that these subscriptions remain active after homebridge has exited (see issue \#5), until they timeout, 30 minutes after they were created or last renewed.
